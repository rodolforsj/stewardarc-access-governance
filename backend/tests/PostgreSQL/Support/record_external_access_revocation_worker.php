...
/**
 * Test-only worker: runs the real RecordExternalAccessRevocation Action in an
 * independent process/connection and reports the outcome as JSON on stdout.
 *
 * Usage: php record_external_access_revocation_worker.php <grantedAccessId> <actorId> [applicationName]
 */
...
if (($argv[3] ?? '') !== '') {
    // Identifies this worker's backend in pg_stat_activity and pg_locks.
    DB::select("select set_config('application_name', ?, false)", [(string) $argv[3]]);
}

try {
    $confirmation = (new RecordExternalAccessRevocation())->execute(
        (string) ($argv[1] ?? ''),
        (string) ($argv[2] ?? ''),
    );

    echo json_encode(['status' => 'recorded', 'id' => $confirmation->id]), PHP_EOL;
