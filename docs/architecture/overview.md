...
The application implementation baseline is recorded in [ADR-006](adr/0006-application-implementation-baseline.md): Eloquent models stay in `App\Models`, future functional operations live in `App\AccessGovernance\Actions` and PostgreSQL-specific concurrency mechanisms in `App\AccessGovernance\Concurrency`; each critical Action owns its own transaction; the advisory lock key is a namespaced SHA-256 derivation passed to `pg_advisory_xact_lock(integer, integer)`; locks are acquired advisory first, then row; and database-dependent tests run against real PostgreSQL. The slices implemented under these namespaces are Request access (UC-001 / RF-002), Decide access request (UC-002 / RF-004 and RF-005), Confirm external access grant (UC-003 / RF-006), Record external access revocation (UC-004 / RF-007) and Follow my requests and accesses (UC-005 / RF-009, RF-010 and RF-011 in the Requester scope).
...
- the concrete PHP mechanism for the read projection transaction and the representation returned by projections (their isolation is decided in ADR-008);
- the concrete PHP helper that obtains the functional transaction instant (the time source itself is decided in ADR-009);
