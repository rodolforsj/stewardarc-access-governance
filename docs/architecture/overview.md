...
- the concrete authentication materialization: identity provider for production and demonstration, Laravel library or package, client authentication method at the token endpoint, session store, lifetime and timeouts, concrete `SameSite` value, the exact encoding of `external_identity_key`, the concrete provisioning mechanism and demonstration identities, and whether a complete local login is demonstrated (the entry boundary is decided in ADR-010 and the authentication baseline in ADR-011);
