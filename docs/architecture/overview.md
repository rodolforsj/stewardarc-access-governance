...
- concrete authentication and session mechanism, identity provider, and the concrete format and provisioning of `external_identity_key` and Actor References (the application entry boundary itself is decided in ADR-010);
...
- detailed API contracts, including endpoint paths, payloads, HTTP status mapping, error envelope, versioning, pagination and the presentation order of projections (only the protected `/api` JSON surface is decided in ADR-010);
