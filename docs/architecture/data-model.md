...
- **Functional time source:** [ADR-009](adr/0009-functional-time-source-baseline.md) makes PostgreSQL `transaction_timestamp()` the canonical source of `access_requests.requested_at`, `decisions.decided_at`, `grant_confirmations.recorded_at` and `revocation_confirmations.recorded_at` and the four mutation Actions now take these instants from it. One instant is used per operation, and a grant confirmation's `recorded_at` is also the base of `valid_until_at`. The application keeps assigning these values explicitly: there are no database defaults or triggers, the column types do not change, and rows recorded before this materialization keep their values. The time component of UUIDv7 identifiers is unrelated to these instants.
...
| `external_identity_key` | `text` | NOT NULL | UNIQUE. Stable key that maps the authenticated external identity to the actor. Opaque to the functional domain and to external clients; its authentication format is described below. |
| `display_name` | `text` | NOT NULL | Name used when presenting the actor. |

For OpenID Connect ([ADR-011](adr/0011-authentication-baseline.md)), `external_identity_key` holds the canonical identity (`iss`, `sub`) as:

```text
oidc:v1:<base64url(iss)>:<base64url(sub)>
```

- `oidc` names the identity kind and `v1` the version of the representation.
- Each claim is encoded on its own, byte for byte, with unpadded Base64URL (RFC 4648 §5); `:` is not part of that alphabet, so the pair is unambiguous.
- Nothing is normalized: no case folding, trimming, trailing-slash handling or URL canonicalization. An empty issuer or subject is not accepted.
- For example, issuer `https://id.example.test` and subject `248289761001` give `oidc:v1:aHR0cHM6Ly9pZC5leGFtcGxlLnRlc3Q:MjQ4Mjg5NzYxMDAx`.
- The key stays UNIQUE. It is neither a secret nor a credential, it is not an authority or role, it holds no token, and it is not exposed by the API.

Actor References are resolved by this key only — never by e-mail or display name — and are pre-provisioned; none is created on first login. The Actor Reference is also the principal of the application's session guard, which keeps only its `id` in the session; the table still has no password or remember-token column.
