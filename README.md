...
> **Status:** documentation baseline, executable project scaffold, persistence layer and the first four application slices (Request access, Decide access request, Confirm external access grant and Record external access revocation) established. There is still no API, no authentication and no user interface.
...
Four application slices are implemented as Actions: **Request access** (UC-001 / RF-002), **Decide access request** (UC-002 / RF-004 and RF-005), **Confirm external access grant** (UC-003 / RF-006) and **Record external access revocation** (UC-004 / RF-007). There are still no endpoints, no authentication and no user interface, so the PostgreSQL suite below is how the functional behavior is exercised and validated.
