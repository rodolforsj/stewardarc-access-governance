...
> **Status:** documentation baseline, executable project scaffold, persistence layer and the first three application slices (Request access, Decide access request and Confirm external access grant) established. There is still no API, no authentication and no user interface.
...
Three application slices are implemented as Actions: **Request access** (UC-001 / RF-002), **Decide access request** (UC-002 / RF-004 and RF-005) and **Confirm external access grant** (UC-003 / RF-006). There are still no endpoints, no authentication and no user interface, so the PostgreSQL suite below is how the functional behavior is exercised and validated.
